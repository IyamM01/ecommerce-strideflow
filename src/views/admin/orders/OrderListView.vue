...
const fetchOrders = async () => {
  ...
  try {
    ...
    if (selectedOrder.value) {
      selectedOrder.value = orders.value.find((order) => order.id === selectedOrder.value?.id) ?? null
    }
  } catch (err: any) {
    error.value = err.response?.data?.message || 'Gagal mengambil data order'
  } finally {
    loading.value = false
  }
}
...
const handleView = async (id: number) => {
  ...
  try {
    ...
    error.value = err.response?.data?.message || 'Gagal mengambil detail order'
  } finally {
    loading.value = false
  }
}
...
const handleDelete = async (id: number) => {
  ...
  try {
    ...
    error.value = err.response?.data?.message || 'Gagal menghapus order'
  } finally {
  ...
  <div class="bg-background text-on-background font-inter min-h-screen flex">
    ...
    <main class="flex-1 md:ml-64 p-margin">
      ...
      <div class="grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,1fr)]">
        <section class="overflow-hidden rounded-3xl border border-surface-container-high bg-surface-container-lowest shadow-sm">
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="bg-surface-container-low/50 text-[11px] font-bold uppercase tracking-widest text-on-surface-variant">
              ...
              <tbody class="divide-y divide-surface-container-high">
                ...
                <tr v-for="order in filteredOrders" :key="order.id" class="hover:bg-surface-container-low transition-colors">
                  <td class="p-5 pl-6">
                  ...
                  <td class="p-5">
                    <div class="flex flex-col">
                      <span class="font-medium text-on-background">{{ order.user?.name || 'Guest' }}</span>
                      <span class="text-xs text-on-surface-variant">{{ order.user?.email || '-' }}</span>
                    </div>
                  </td>
        ...
        <aside class="rounded-3xl border border-surface-container-high bg-surface-container-lowest shadow-sm">
          <div class="border-b border-surface-container-high px-6 py-5">
          ...
          <div v-if="selectedOrder" class="grid gap-6 p-6">
            ...
            <div class="grid gap-3">
              <h3 class="text-sm font-bold uppercase tracking-wider text-on-surface-variant">Items</h3>
              ...
              >
                <div class="flex items-start gap-3">
                  ...
                  <div class="min-w-0 flex-1">
                    <p class="font-semibold text-on-background">{{ item.product?.name || 'Produk terhapus' }}</p>
            ...
            <div class="grid gap-2 rounded-2xl border border-surface-container-high bg-surface-container-low p-4">
              <div class="flex items-center justify-between gap-3">
              ...
              <div class="flex items-center justify-between gap-3">
                ...
                <span class="text-sm text-on-background">{{ selectedOrder.payment?.provider || '-' }}</span>
              </div>
              <div class="flex items-center justify-between gap-3">
                <span class="text-sm font-semibold text-on-surface-variant">Invoice</span>
                <span class="text-right text-sm text-on-background">{{ selectedOrder.payment?.invoice_id || '-' }}</span>
              </div>
              <div class="flex items-center justify-between gap-3 border-t border-surface-container-high pt-2">
                <span class="text-sm font-semibold text-on-surface-variant">Total</span>
                <span class="text-lg font-bold text-on-background">{{ formatCurrency(selectedOrder.total_price) }}</span>
              </div>
            </div>
          </div>
