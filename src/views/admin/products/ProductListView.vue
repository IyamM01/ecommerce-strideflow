  ...
  <div class="bg-background text-on-background font-inter min-h-screen flex">
    ...
    <main class="flex-1 md:ml-64 p-margin">
      ...
      <div class="bg-surface-container-lowest rounded-2xl shadow-sm border border-surface-container-high overflow-hidden">
        <!-- Table Header / Search -->
        <div class="p-6 border-b border-surface-container-high flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div class="flex gap-2">
            <button class="p-3 rounded-xl bg-surface-container-low text-on-surface-variant hover:bg-surface-container-high transition-colors">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-surface-container-high">
              <tr v-if="productStore.loading" v-for="i in 5" :key="i" class="animate-pulse">
                 <td colspan="6" class="p-8 text-center text-on-surface-variant">Loading products...</td>
              </tr>
              ...
              >
                <td class="p-5 pl-8">
                  <div class="flex items-center gap-4">
                    <div class="w-14 h-14 rounded-xl bg-surface-container-high overflow-hidden flex-shrink-0 border border-surface-container-highest">
                      <img :src="product.image_url || 'https://placehold.co/160x160?text=No+Image'" :alt="product.name" class="w-full h-full object-cover" />
                    </div>
                    <div class="flex flex-col min-w-0">
                      <span class="font-bold text-on-background truncate">{{ product.name }}</span>
                      <span class="text-xs text-on-surface-variant uppercase tracking-wider font-medium">{{ product.sku || 'NO SKU' }}</span>
                    </div>
                  </div>
                </td>

                <td class="p-5">
                  <span class="px-3 py-1 bg-surface-container-high text-on-surface text-xs font-bold rounded-full">
                  ...
                  <div class="flex flex-col gap-1">
                    ...
                    <div class="w-24 h-1.5 bg-surface-container-high rounded-full overflow-hidden">
                      <div
                        ...
                        :class="product.stock <= 5 ? 'bg-error' : product.stock <= 15 ? 'bg-amber-500' : 'bg-primary'"
                ...
                <td class="p-5">
                  <span
                    ...
                    :class="product.stock > 0 ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'"
                  >
              ...
              <tr v-if="!productStore.loading && filteredProducts.length === 0">
                <td colspan="6" class="p-20 text-center">
                  <div class="flex flex-col items-center gap-4 text-on-surface-variant">
                    ...
                    <p class="font-medium">No products found. Try a different search or add a new product.</p>
                  </div>
                </td>
              </tr>
            </tbody>
